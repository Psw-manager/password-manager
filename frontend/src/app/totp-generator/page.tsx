...
export default function TotpGeneratorPage() {
  ...
  const [isGenerating, setIsGenerating] = useState(false); // State to track if generation is active
  ...
  const copyToClipboard = () => {
    if (totp) {
      ...
      navigator.clipboard.writeText(totp).catch((err) => {
  ...
  useEffect(() => {
    if (!secretKey || tokenPeriod <= 0 || digits <= 0) return;

    // Trigger TOTP generation when secretKey changes
    generateTotp();
  }, [secretKey, digits, tokenPeriod]); // Dependencies: regenerate TOTP when secretKey, digits, or tokenPeriod changes

  useEffect(() => {
    if (tokenPeriod <= 0 || !isGenerating) return;
    ...
    const timer = setInterval(() => {
      setTimeElapsed((prevTime) => {
        ...
        if (newProgress >= 100) {
          generateTotp(); // Regenerate TOTP after period ends
          return 0; // Reset time elapsed
        }
    ...
    <div className="flex justify-center items-center h-5/6">
      <Card className="w-3/5">
        ...
        <CardFooter className="flex justify-between items-center">
          {/* Removed the "Generate TOTP" button */}
